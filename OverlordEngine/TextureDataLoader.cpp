...
TextureData* TextureDataLoader::LoadContent(const std::wstring& assetFile)
{
	...
	if (generateMips)
	{
		DirectX::ScratchImage* imageMips = new DirectX::ScratchImage();
		int flags = TEX_FILTER_POINT | TEX_FILTER_MIRROR | TEX_FILTER_FORCE_NON_WIC | TEX_FILTER_SEPARATE_ALPHA;
#if defined(_M_IX86)
		hr = GenerateMipMaps(image->GetImages(), image->GetImageCount(), image->GetMetadata(), flags, 0, *imageMips);
#elif defined(_M_X64)
		hr = GenerateMipMaps(image->GetImages(), image->GetImageCount(), image->GetMetadata(), static_cast<TEX_FILTER_FLAGS>(flags), 0, *imageMips);
#endif
		SafeDelete(image);
		image = imageMips;
	}
