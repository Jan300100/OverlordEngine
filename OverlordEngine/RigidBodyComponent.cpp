...
#elif defined(_M_X64)
		physx::PxShape* shape = m_pActor->createShape(*collider->m_Geometry, collider->m_Material);
		shape->setLocalPose(collider->m_LocalPose);
...
#elif defined(_M_X64)
		reinterpret_cast<physx::PxRigidDynamic*>(m_pActor)->setRigidBodyFlag(
			physx::PxRigidBodyFlag::eKINEMATIC, m_isKinematic);
#endif
	}
}
...
void RigidBodyComponent::SetDensity(float density) const
{
	...
	{
		physx::PxRigidBodyExt::updateMassAndInertia(static_cast<physx::PxRigidDynamic&>(*m_pActor), (physx::PxF32)density);
	}
#endif
}
