...
bool EffectHelper::BuildInputLayout(ID3D11Device* pDevice, ID3DX11EffectTechnique* pTechnique, ID3D11InputLayout** pInputLayout, std::vector<ILDescription>& inputLayoutDescriptions, UINT& inputLayoutSize, UINT& inputLayoutID)
{
	...
	for (UINT i = 0; i < effectShaderDesc.NumInputSignatureEntries; ++i)
	{
		...
		ilDescription.Classification = D3D11_INPUT_PER_VERTEX_DATA;
		//Semantic Format
		...
		else if (strcmp(signParDesc.SemanticName, "SV_InstanceID") == 0)
		{
			ilDescription.SemanticType = ILSemantic::INSTANCEID;
			ilDescription.Classification = D3D11_INPUT_PER_INSTANCE_DATA;
		}
		else if (strcmp(signParDesc.SemanticName, "INSTANCE1F") == 0)
		{
			ilDescription.SemanticType = ILSemantic::INSTANCE1F;
			ilDescription.Classification = D3D11_INPUT_PER_INSTANCE_DATA;
		}
		else if (strcmp(signParDesc.SemanticName, "INSTANCE2F") == 0)
		{
			ilDescription.SemanticType = ILSemantic::INSTANCE2F;
			ilDescription.Classification = D3D11_INPUT_PER_INSTANCE_DATA;
		}
		else if (strcmp(signParDesc.SemanticName, "INSTANCE3F") == 0)
		{
			ilDescription.SemanticType = ILSemantic::INSTANCE3F;
			ilDescription.Classification = D3D11_INPUT_PER_INSTANCE_DATA;
		}
		else if (strcmp(signParDesc.SemanticName, "INSTANCE4F") == 0)
		{
			ilDescription.SemanticType = ILSemantic::INSTANCE4F;
			ilDescription.Classification = D3D11_INPUT_PER_INSTANCE_DATA;
		}
		...
		if (ilDescription.Classification == D3D11_INPUT_PER_INSTANCE_DATA)
		{
			D3D11_INPUT_ELEMENT_DESC inputLayout = { signParDesc.SemanticName, signParDesc.SemanticIndex, ilDescription.Format,1 , instanceSize, ilDescription.Classification,1 };
			layoutDesc.push_back(inputLayout);
			instanceSize += ilDescription.Offset;
		}
		else
		{
			D3D11_INPUT_ELEMENT_DESC inputLayout = { signParDesc.SemanticName, signParDesc.SemanticIndex, ilDescription.Format,0 , inputLayoutSize, ilDescription.Classification,0};
			layoutDesc.push_back(inputLayout);
	...
	HRESULT hr = pDevice->CreateInputLayout(static_cast<D3D11_INPUT_ELEMENT_DESC*>(&layoutDesc[0]), (UINT)layoutDesc.size(), PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, pInputLayout);
