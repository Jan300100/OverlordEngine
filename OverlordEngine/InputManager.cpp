...
void InputManager::Update()
{
	...
	int maxMouseMS = 10;
	if (m_MouseMovement.x > maxMouseMS)
	{
		m_MouseMovement.x = maxMouseMS;
	}
	if (m_MouseMovement.x < -maxMouseMS)
	{
		m_MouseMovement.x = -maxMouseMS;
	}
	if (m_MouseMovement.y > maxMouseMS)
	{
		m_MouseMovement.y = maxMouseMS;
	}
	if (m_MouseMovement.y < -maxMouseMS)
	{
		m_MouseMovement.y = -maxMouseMS;
...
{
	UINT pi = static_cast<UINT>(playerIndex);
	if (!m_ConnectedGamepads[pi])
		return false;

	if (previousFrame)
		return (m_OldGamepadState[pi].Gamepad.wButtons & button) != 0;
	return (m_CurrGamepadState[pi].Gamepad.wButtons & button) != 0;
}
...
DirectX::XMFLOAT2 InputManager::GetThumbstickPosition(bool leftThumbstick, GamepadIndex playerIndex)
{
	...
	if (leftThumbstick)
	{
		pos = DirectX::XMFLOAT2(m_CurrGamepadState[pi].Gamepad.sThumbLX, m_CurrGamepadState[pi].Gamepad.sThumbLY);
	...
	{
		pos = DirectX::XMFLOAT2(m_CurrGamepadState[pi].Gamepad.sThumbRX, m_CurrGamepadState[pi].Gamepad.sThumbRY);
...
float InputManager::GetTriggerPressure(bool leftTrigger, GamepadIndex playerIndex)
{
	...
	if (leftTrigger)
	{
		return m_CurrGamepadState[pi].Gamepad.bLeftTrigger / 255.0f;
	}
	return m_CurrGamepadState[pi].Gamepad.bRightTrigger / 255.0f;
}
...
void InputManager::ForceMouseToCenter(bool keepMouseCentered)
{
	m_ForceToCenter = keepMouseCentered;
