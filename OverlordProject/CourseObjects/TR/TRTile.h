...
{
public:
	TRTile(const DirectX::XMINT2& position, const DirectX::XMINT2& direction, int initRoadRef = 0, int initFreeRef = 0, size_t level = 0);
	...
	DirectX::XMINT2 GetDirection() const { return m_Direction; }
	DirectX::XMINT2 GetPosition() const { return m_Position; }
...
protected:
	...
	Type m_Type;
	std::vector< Instance<DirectX::XMFLOAT4X4>> m_Details;
	...
	DirectX::XMINT2 m_Direction;
private:
	void GenerateProps();
	static size_t NR_TILES;
	int m_FreeRef;
	int m_RoadRef;
	Instance<DirectX::XMFLOAT2>* m_pGroundQuad;
