...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	return diffuseColor * saturate(dot(-normal, gLightDirection));
}
...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	const float observedArea = dot(-normal, gLightDirection);
	if (observedArea > 0.f)
	{
	...
	{
		specularIntensity = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
	}
	return specColor * specularIntensity;
}
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	if (observedArea > 0.f)
	{
		float dotResult = dot(reflect(gLightDirection, normal), viewDirection);
		specularColor = float4(1.f, 1.f, 1.f, 1.f) * pow(abs(dotResult), gShininess); //calc phong color
				
		//calculate Final Color
		specularColor  *= observedArea;
	}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	if (gUseFresnelFalloff)
	{
		...
		fresnel = saturate(pow(fresnel, gFresnelPower)) * gFresnelMultiplier;
		float fresnelMask = (1-saturate(dot(float3(0,-1,0), normal)));
		...
		{
			return 	gColorFresnel * fresnel * fresnelMask;
		}	
	}
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	if (gUseTextureEnvironment)
	{
		...
		reflected = gCubeEnvironment.Sample(gTextureSampler, reflected);
		reflected *= gReflectionStrength;
		
		//refracted
		float3 refracted = refract(viewDirection, normal, gRefractionIndex);
		refracted = gCubeEnvironment.Sample(gTextureSampler, refracted);
		refracted *= gRefractionStrength;
