...
VS_OUTPUT VS(VS_INPUT input){
	...
	for (int i = 0; i < 4; i++)
	{
		...
		if (boneIndex >= 0) //vertex is attached to this bone
		{
			...
			transformedPosition += mul(input.blendWeights[i],mul(float4(input.pos,1),gBones[boneIndex]));
			//only rotate normmal
	...
	output.pos = mul ( float4(transformedPosition), gWorldViewProj ); //Non skinned position
