             ...
             const OutputPatch<HullOut, 3> tri)
{
    ...
    float4 worldPos = mul(float4(output.Position), gMatrixWorld);
    
    //lPos here: or we get shadows because of the displacement
    output.lPos = mul(output.Position,  mul(gMatrixWorld, gViewProj_Light));

    //update texcoords based on worldspace
    output.TexCoord = float2(worldPos.x / gWorldUVScale, worldPos.z / gWorldUVScale);
